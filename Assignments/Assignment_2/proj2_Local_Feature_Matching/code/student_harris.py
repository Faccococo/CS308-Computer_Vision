...
def ANMS (x , y, r, maximum):
	...
	param:
		...
		-> maximum: is the no of corners that are required
	'''
	i = 0
	j = 0
	NewList = []
	while i < len(x):
		minimum = 1000  #random large value
...
def noANMS (x , y):
	...
	while i < len(x):
		FirstCoordinate, SecondCoordinate = x[i], y[i]
		NewList.append([FirstCoordinate, SecondCoordinate])
		i = i + 1
	return NewList


def get_interest_points(image,img_name, feature_width, feature_num):
	alpha = 0.04
	threshold = 10000
	XCorners = []
	YCorners = []
...
#    NewCorners = noANMS(XCorners, YCorners)
	NewCorners = ANMS(XCorners, YCorners, RValues, feature_num)
	NewCorners = np.asarray(NewCorners)
	print('before ANMS '+img_name+'has corners num:'+str(len(XCorners)))
	print('after ANMS '+img_name+'has corners num:'+str(len(NewCorners)))
